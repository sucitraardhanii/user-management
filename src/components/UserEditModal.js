...
export default function UserEditModal({ opened, onClose, userData, onSubmit }) {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      ...
      try {
        ...
        if (
          userData?.jabatan &&
          !jabatan.some((j) => j.value === userData.jabatan)
        ) {
        ...
        if (
          userData?.kantor &&
          !semuaKantor.some((k) => k.value === userData.kantor)
        ) {
  ...
  useEffect(() => {
    if (userData && jabatanOptions.length > 0) {
      const jabatanCode =
        jabatanOptions.find((j) => j.label === userData.jabatan)?.value || "";
      ...
      if (userData) {
        form.setValues({
          ...
          status_pegawai:
            userData.status_pegawai === 4 ? "Non Organik" : "Organik",
        });
      }
    }
      ...
      <form onSubmit={form.onSubmit(handleSubmit)}>
        <Stack>
          ...
          <Autocomplete
            ...
            value={
              kantorOptions.find((opt) => opt.value === form.values.kantor)
                ?.label || ""
            }
            onChange={(val) => {
              setKantorInput(val);
